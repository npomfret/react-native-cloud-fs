...
_sourceUri_ (required): any url or **absolute** file path, e.g:
 * `/foo/bar/file.txt`
...
_destinationPath_ (required): a **relative** path (including a filename) under which the file will be placed, e.g:
 * `my-cloud-text-file.txt`
 * `foo/bar/my-cloud-text-file.txt`
 
_mimeType_ (optional):  a mime type to store the file with (android only) 
 * `text/plain`
 * `application/json`
 * `image/jpeg`
